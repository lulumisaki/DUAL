...
# from evaluate_stable_matching import evaluate
...
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features = load_data("data/dbp_yg_15k_V1/",train_ratio=0.30)
...
def get_embedding(index_a, index_b, vec=None, weight=0, use_bert=True):
    if vec is None:
    ...
    if use_bert == True:
        lit_sim = bert.get_bert_embedding("data/en_de_15k_V1/ent_attr_1", "data/en_de_15k_V1/ent_attr_2")
        return Lvec, Rvec, weight * lit_sim
    else:
        return Lvec, Rvec, np.zeros(shape=(10500, 768))
...
def get_trgat(node_hidden, rel_hidden, triple_size=triple_size, node_size=node_size, rel_size=rel_size, dropout_rate=0,
    ...
    # self_nodes = Input(shape=(None, 1), dtype='int64')
    # neight_nodes = Input(shape=(None, 1), dtype='int64')
...
for turn in range(1):
    for i in trange(epoch):
        ...
        if i + 1 == 20:
            Lvec, Rvec, lit_sim = get_embedding(dev_pair[:, 0], dev_pair[:, 1], weight=w_lit, use_bert=False)
            evaluater.test(Lvec, Rvec, lit_sim)
    #     new_pair = []
